...
router = APIRouter(prefix="/api/team-dashboard", tags=["team-dashboard"], dependencies=[Depends(_audit_dashboard_activity)])
...
def _is_valid_team_name(value: Any) -> bool:
    """Exclude placeholder/no-team markers from selectable team options."""
    if value is None:
        return False
    try:
        if pd.isna(value):
...
def aggregate_team_kpis(team_members: List[str], team_members_df: pd.DataFrame, evaluator: KppEvaluator, roles_df: pd.DataFrame, period: str, as_of_date: Optional[str] = None) -> Dict[str, Any]:
    """Aggregate KPI data for a team."""
    ...
    for kpi in sorted(all_kpis, key=lambda x: int(x[1:])):
        ...
        # Aggregate values from team members - only count members where KPI is applicable
        team_values = []
        applicable_count = 0
        applicable_members = []
        for member in team_members:
            # Get member's roles
            member_info = team_members_df[team_members_df['Name'] == member]
            ...
            if kpi in SECURITY_SCAN_KPIS:
                ...
                # k1 and equivalents: collect from ALL team members (Copilot is used
                # across the whole team, so absent members count as 0 in the average)
            ...
            if member_value is not None and not pd.isna(member_value):
                try:
                    team_values.append(float(member_value))
                except (ValueError, TypeError):
                    continue
        ...
        if kpi == 'k222':
            team_avg, _, total_story_points, _ = compute_k222_ratio_for_members(
            ...
            # Compute as sum(k227)/sum(k1) for the team to avoid sentinel 9999
            # values from individual members with no Copilot usage skewing the average.
            team_avg = compute_k268_ratio_for_members(
                resources_file=evaluator.resources_file,
            ...
            if kpi in K1_EQUIVALENT_KPIS:
                # k1 / equivalents: divide by full team size so that members
                # without Copilot data correctly pull the average down.
                team_avg = sum(team_values) / len(team_members)
            else:
        ...
        rog_status = calculate_rog_status(team_avg, comparison_target, goal_type)
        
        # Calculate percentage against prorated target
        percentage = (team_avg / comparison_target * 100) if comparison_target != 0 else 0
...
@router.get("/by-name")
def get_team_dashboard_by_name(
    team_name: str,
    period: str = "Annual",
    as_of_date: Optional[str] = None,
    current_user: TokenData = Depends(get_current_user)
):
    """Slash-safe team dashboard endpoint using query parameter team_name."""
    return get_team_dashboard(team_name, period, as_of_date, current_user)
...
def get_team_dashboard(
    ...
    """
    Get comprehensive dashboard data for a specific team.

    Returns team profile, aggregated KPI performance, and ROG status.
    """
    try:
        # Set up paths
        ...
        if viewer.role not in ["Admin", "Admin Viewer", "API User", "Team Manager", "Lead"]:
            raise HTTPException(status_code=403, detail="Insufficient permissions for team dashboard")

        if viewer.role in ["Team Manager", "Lead"] and team_name not in (viewer.team_ids or []):
            raise HTTPException(status_code=403, detail="You can only view team dashboards for your teams")
        ...
        try:
            ...
            snapshot_payload = snapshot_service.get_team_payload(team_name, period, as_of_date)
            if snapshot_payload:
                return snapshot_payload
        except Exception as exc:
            logger.warning("Snapshot lookup failed for team dashboard (%s): %s", team_name, exc)
        ...
        # Get unique scrums for this team
        team_scrums = team_members_df['Scrum'].dropna().unique().tolist()

        # Get member details with scrum info
        member_details = []
        ...
        # Aggregate team KPI data
        team_data = aggregate_team_kpis(team_members, team_members_df, evaluator, roles_df, period, as_of_date)
        ...
        logger.error(f"Error loading team dashboard: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.get("/list/teams")
def list_teams(current_user: TokenData = Depends(get_current_user)):
    """Get list of all teams for selection dropdown."""
    try:
        resources_file = project_root / 'config' / 'Resources.csv'
        resources_df = _normalize_resources_df(pd.read_csv(resources_file))
        ...
        if viewer.role not in ["Admin", "Admin Viewer", "API User", "Team Manager", "Lead"]:
            raise HTTPException(status_code=403, detail="Insufficient permissions for team dashboards")
        
        # Get unique teams
        teams = sorted({
            str(team).strip()
            for team in resources_df['Team'].unique().tolist()
            if _is_valid_team_name(team)
        })

        if viewer.role in ["Team Manager", "Lead"]:
            allowed = set(viewer.team_ids or [])
            teams = [team for team in teams if team in allowed]
        
        # Count members in each team
        team_list = []
        for team in teams:
            member_count = len(resources_df[resources_df['Team'] == team])
            team_list.append({
                "name": str(team),
