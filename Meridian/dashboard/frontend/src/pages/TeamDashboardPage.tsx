...
import { teamDashboardApi, TeamOption, KPIPerformance, fetchAvailableDates } from '../services/teamDashboardApi'
...
export default function TeamDashboardPage() {
  ...
  useEffect(() => {
    const loadTeams = async () => {
      ...
      try {
        const response = await teamDashboardApi.listTeams()
        setTeams(response.teams)
        
        // Check if team parameter is in URL
        const teamParam = searchParams.get('team')
        if (teamParam && response.teams.length > 0) {
          const team = response.teams.find(t => t.name === teamParam)
          if (team) {
  ...
  // Load dashboard data when team or period or date changes
  useEffect(() => {
    if (!selectedTeam) return
    const timer = setTimeout(() => { loadDashboardData() }, 400)
    return () => clearTimeout(timer)
  }, [selectedTeam, period, asOfDate])
  ...
  const loadDashboardData = async () => {
    ...
    setError(null)
    try {
      const data = await teamDashboardApi.getTeamDashboard(selectedTeam.name, period, asOfDate ? asOfDate.replace(/-/g, '') : '')
      ...
      <Paper sx={{ p: 3 }}>
        <Box sx={{ mb: 3 }}>
          ...
          <Typography variant="body2" color="text.secondary">
            Aggregated performance view for teams
          </Typography>
        </Box>

        {/* Team Selection and Period Filter */}
        <Box sx={{ mb: 3 }}>
        ...
        {!loading && dashboardData && (
          <>
            {/* Team Profile Accordion - Collapsible */}
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, width: '100%' }}>
                  ...
                  <Box>
                    <Typography variant="h6" fontWeight={600}>
                      {dashboardData.team.name} - Team Details
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                      {dashboardData.team.member_count} team members • Click to expand
                    </Typography>
                  </Box>
                </Box>
              </AccordionSummary>
                    ...
                    <Grid item xs={12} md={6}>
                      <Card variant="outlined">
                        <CardContent>
                          ...
                          {dashboardData.team.primary_role_distribution && dashboardData.team.primary_role_distribution.length > 0 ? (
                            <>
                              <ResponsiveContainer width="100%" height={250}>
                                <PieChart>
                                  <Pie
                                    data={dashboardData.team.primary_role_distribution}
                                    dataKey="count"
                                    nameKey="role"
                                    cx="50%"
                                    cy="50%"
                                    outerRadius={80}
                                  >
                                    {dashboardData.team.primary_role_distribution.map((_: any, index: number) => (
                              ...
                              <Box sx={{ mt: 2 }}>
                                {dashboardData.team.primary_role_distribution.map((item: any, index: number) => (
                    ...
                    <Grid item xs={12} md={6}>
                      <Card variant="outlined">
                        <CardContent>
                          ...
                          {dashboardData.team.secondary_role_distribution && dashboardData.team.secondary_role_distribution.length > 0 ? (
                            <>
                              <ResponsiveContainer width="100%" height={250}>
                                <PieChart>
                                  <Pie
                                    data={dashboardData.team.secondary_role_distribution}
                                    dataKey="count"
                                    nameKey="role"
                                    cx="50%"
                                    cy="50%"
                                    outerRadius={80}
                                  >
                                    {dashboardData.team.secondary_role_distribution.map((_: any, index: number) => (
                                      <Cell key={`cell-${index}`} fill={CHART_COLORS[index % CHART_COLORS.length]} />
                                    ))}
                                  </Pie>
                                  <Tooltip />
                              ...
                              <Box sx={{ mt: 2 }}>
                                {dashboardData.team.secondary_role_distribution.map((item: any, index: number) => (
                                  <Box key={index} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                                    <Box sx={{ width: 12, height: 12, bgcolor: CHART_COLORS[index % CHART_COLORS.length], borderRadius: '50%' }} />
                                    <Typography variant="body2" sx={{ fontSize: '0.75rem' }}>
                                      {item.role}: {item.count}
                  ...
                  <Grid container spacing={2}>
                    <Grid item xs={12} md={4}>
                      ...
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                        {dashboardData.team.scrums && dashboardData.team.scrums.length > 0 ? (
                          dashboardData.team.scrums.map((scrum: string) => (
                            <Chip 
                              key={scrum} 
                              label={scrum} 
                              size="small" 
                              icon={<GroupsIcon />}
                              onClick={() => navigate(`/dashboard/scrum?scrum=${encodeURIComponent(scrum)}`)}
                              sx={{ cursor: 'pointer' }}
                              color="primary"
                              variant="outlined"
                            />
                          ...
                          <Typography variant="body2" color="text.secondary">No scrums</Typography>
                        )}
                      </Box>
                    </Grid>
                    <Grid item xs={12} md={8}>
                      <Typography variant="body2" fontWeight={600} gutterBottom>
                        Team Members ({dashboardData.team.member_count}):
                      </Typography>
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                        {dashboardData.team.member_details && dashboardData.team.member_details.map((member: any) => (
                          <Chip 
                            key={member.sapid || member.name} 
                            label={`${member.name} (${member.scrum})`} 
                            size="small" 
                            variant="outlined"
        ...
        {!loading && !dashboardData && !error && (
          <Box sx={{ textAlign: 'center', py: 8 }}>
            ...
            <Typography variant="h6" color="text.secondary">
              Select a team to view their dashboard
            </Typography>
          </Box>
        )}
      </Paper>
