#!/usr/bin/env python3
"""
KPP K2 - End-to-End Bug Cycle Time
Calculates end-to-end bug cycle time from Created to Done/Invalid state, aggregated by Scrum/Team.
"""
...
def get_description():
    ...
    return "End-to-End Bug Cycle Time - Calculates bug cycle time from Created to Done/Invalid state, aggregated by Scrum/Team"
...
def calculate_bug_cycle_time(key, created_date, updated_date, history_df, end_states=['Done', 'Invalid']):
    """
    Calculate end-to-end cycle time for a bug from Created to final state.
    ...
    Args:
        ...
        end_states: List of acceptable end states (Done, Invalid)
...
def k2(resources_file, jira_issues_file, output_dir, fiscal_start_month=4, current_date=None):
    """
    K2: End-to-End Bug Cycle Time aggregated by Scrum/Team.
    ...
    """
    output_file = os.path.join(output_dir, 'k2-data.csv')
    ...
    print(f"\n{'='*60}")
    print(f"KPP K2 - End-to-End Bug Cycle Time")
    ...
    print("Calculating end-to-end cycle times per individual...")
    ...
    for idx, bug in merged.iterrows():
        cycle_time, reached_end, completion_date = calculate_bug_cycle_time(
            ...
            bug['Updated'],
            history_df,
            end_states=['Done', 'Invalid']  # Only Done/Invalid for K2
        )
