#!/usr/bin/env python3
"""
KPP K301 - DAST Gating Issues
Counts total DAST security issues (High + Medium severity only) per team,
derived from the DAST HTML reports downloaded by the security_scan_fetch job.

Data source:  output/scans/dast/<project_id>_dast.html
              (HTML generated by HCL AppScan DAST; section "Summary of security issues")
...
Output file: output/k301-data.csv
...
def get_description() -> str:
    ...
    return (
        "DAST Gating Issues – Total DAST security issues (High + Medium severity) per team, "
        "aggregated from AppScan DAST HTML reports. "
        "Source: 'Summary of security issues' section of each project's DAST report. "
        "If a team has multiple scan projects, counts are summed."
    )


def _parse_dast_html(html_path: str) -> int:
    """
    Parse an AppScan DAST HTML report and return the total of
    ...
    if not os.path.exists(html_path):
        print(f"    ⚠ DAST report not found: {html_path}")
        return 0
...
def _build_team_scores(scan_config: dict, scans_dast_dir: Path) -> tuple[dict[str, int], dict[str, list[str]]]:
    """
    Return:
      - team_scores: {team_name: total_dast_count}
    ...
    for project in scan_config.get("projects", []):
        ...
        # Find the DAST report filename for this project
        dast_entry = next((r for r in reports if r.get("type") == "dast"), None)
        if not dast_entry:
            print(f"  ⚠ No DAST report configured for project '{pid}' — skipping")
            continue

        html_path = str(scans_dast_dir / dast_entry["filename"])
        count = _parse_dast_html(html_path)
...
def k301(resources_file: str, output_dir: str, fiscal_start_month: int = 4,
         current_date: date | None = None) -> bool:
    """
    K301: DAST Gating Issues aggregated at Team level, propagated to all
    members of the team.
    ...
    """
    output_file = os.path.join(output_dir, "k301-data.csv")
    ...
    print(f"\n{'='*60}")
    print(f"KPP K301 - DAST Gating Issues")
    ...
    # ── Load scan configuration and parse DAST HTML reports ──────────────────
    ...
    if not scan_config:
        ...
        scans_dast_dir = Path(output_dir) / "scans" / "dast"
        print(f"  Parsing DAST reports from: {scans_dast_dir}")
        team_scores, team_sources = _build_team_scores(scan_config, scans_dast_dir)
        print(f"  Team DAST scores: {team_scores}")
        if team_sources:
    ...
    print("Loading historical k301 data for median computation...")
    ...
    print(f"    Teams with DAST data: {sorted(team_scores.keys())}")
    print(
        f"    Teams with no DAST configuration: "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run KPI K301 – DAST Gating Issues")
    parser.add_argument(
        "--project-root",
