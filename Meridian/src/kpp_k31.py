#!/usr/bin/env python3
"""
KPP K31 - Database Bugs Count
Counts bugs tagged with database-related labels at Team level.
"""
...
def get_description():
    ...
    return "Database Bugs Count - Counts bugs tagged with database-related labels at Team level"
...
def contains_database_label(labels_str, db_labels_normalized):
    """
    Check if any database-related label exists in the Labels column.
    
    Args:
        labels_str: String containing comma-separated labels
        db_labels_normalized: Set of normalized database labels to check
    
    Returns:
        bool: True if any database label is found
    """
    ...
    # Check if any normalized label matches our database labels
    for label in labels:
        if label in db_labels_normalized:
            return True
    
    return False


def count_database_bugs_by_team(jira_issues_df, resources_df, team_name, 
                                 current_date, fiscal_start_month=4):
    """
    Count bugs with database-related labels for a specific team.
    ...
    """
    # Database-related labels (including variations and typos)
    db_labels = [
        'CH-DB-Support',
        'application-db-design',
        'application-db-functional',
        'application_db_funtional',  # typo variant
        'application_db_performance',
        'bug-db-datatype-range',
        'bug-db-functional',
        'bug-db-performance',
        'db',
        'research-db-analysis'
    ]
    
    # Normalize all database labels for comparison
    db_labels_normalized = set(normalize_label(l) for l in db_labels)
    ...
    # Filter by database labels
    bugs_df = bugs_df[bugs_df['Labels'].notna()].copy()
    bugs_df['has_db_label'] = bugs_df['Labels'].apply(
        lambda x: contains_database_label(x, db_labels_normalized)
    )
    bugs_df = bugs_df[bugs_df['has_db_label']].copy()
    
    # If no database bugs found for this team, return zeros
    if len(bugs_df) == 0:
        return {
            'Weekly': 0,
            'Monthly': 0,
...
def k31(resources_file, jira_issues_file, output_dir, fiscal_start_month=4, current_date=None):
    """
    K31: Count database bugs at team level and generate k31-data.csv output.
    ...
    """
    output_file = os.path.join(output_dir, 'k31-data.csv')
    ...
    try:
        ...
        team_counts = {}
        for team in teams:
            counts = count_database_bugs_by_team(
        ...
        print(f"  ✓ {total_teams_with_bugs} teams with database bugs")
        
        return True
        
    except Exception as e:
        print(f"  ✗ Error in K31: {e}")
        import traceback
        traceback.print_exc()
        return False
