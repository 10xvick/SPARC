#!/usr/bin/env python3
"""
KPP K68 - Performance Bugs Count
Counts bugs tagged with performance-related labels at Team level.
"""
...
def get_description():
    ...
    return "Performance Bugs Count - Counts bugs tagged with performance-related labels at Team level"
...
def contains_performance_label(labels_str, performance_labels_normalized):
    """
    Check if any performance-related label exists in the Labels column.
    
    Args:
        labels_str: String containing comma-separated labels
        performance_labels_normalized: Set of normalized performance labels to check
    
    Returns:
        bool: True if any performance label is found
    """
    ...
    # Check if any normalized label matches our performance labels
    for label in labels:
        if label in performance_labels_normalized:
            return True
    
    return False


def count_performance_bugs_by_team(jira_issues_df, resources_df, team_name, 
                                    current_date, fiscal_start_month=4):
    """
    Count bugs with performance-related labels for a specific team.
    ...
    """
    # Performance-related labels (including variations and typos)
    performance_labels = [
        'OOSM-performance-scalibility-Sprint-19-DEV',
        'Perf_Test',
        'Performance',
        'application-performance',
        'application_db_performance',
    ...
    # Normalize all performance labels for comparison
    performance_labels_normalized = set(normalize_label(l) for l in performance_labels)
    ...
    bugs_df['has_performance_label'] = bugs_df['Labels'].apply(
        lambda x: contains_performance_label(x, performance_labels_normalized)
    )
    bugs_df = bugs_df[bugs_df['has_performance_label']].copy()
    
    # If no performance bugs found for this team, return zeros
    if len(bugs_df) == 0:
        return {
            'Weekly': 0,
            'Monthly': 0,
...
def k68(resources_file, jira_issues_file, output_dir, fiscal_start_month=4, current_date=None):
    """
    K68: Count performance bugs at team level and generate k68-data.csv output.
    ...
    """
    output_file = os.path.join(output_dir, 'k68-data.csv')
    ...
    try:
        ...
        team_counts = {}
        for team in teams:
            counts = count_performance_bugs_by_team(
        ...
        print(f"  ✓ {total_teams_with_bugs} teams with performance bugs")
        
        return True
        
    except Exception as e:
        print(f"  ✗ Error in K68: {e}")
        import traceback
        traceback.print_exc()
        return False
