#!/usr/bin/env python3
"""
KPP K77 - Penetration Testing Bugs Count
Counts bugs tagged with penetration testing-related labels at Team level.
"""
...
def get_description():
    ...
    return "Penetration Testing Bugs Count - Counts bugs tagged with penetration testing and vulnerability-related labels at Team level"


def normalize_label(label):
    """
    Normalize label to handle typos and variations.
    Converts underscores to hyphens and makes lowercase for comparison.
    """
...
def contains_pen_test_label(labels_str, pen_test_labels_normalized):
    """
    Check if any penetration testing-related label exists in the Labels column.
    
    Args:
        labels_str: String containing comma-separated labels
        pen_test_labels_normalized: Set of normalized pen test labels to check
    
    Returns:
        bool: True if any pen test label is found
    """
    ...
    # Check if any normalized label matches our pen test labels
    for label in labels:
        if label in pen_test_labels_normalized:
            return True
    
    return False


def count_pen_test_bugs_by_team(jira_issues_df, resources_df, team_name, 
                                 current_date, fiscal_start_month=4):
    """
    Count bugs with penetration testing and vulnerability-related labels for a specific team.
    ...
    """
    # Penetration testing and vulnerability-related labels
    pen_test_labels = [
        'bug-ui-penetration',
        # Vulnerability-related labels
        'bug-ui-vulnerability',
        'Security_Vulnerabilities',
        'application-security-vulnerability-scan',
        'application-security-vulnerability-scan.',  # with trailing dot
        'vulnerability'
        # Note: 'Pending' is excluded as it's not specifically a pen testing label
    ]
    
    # Normalize all pen test labels for comparison
    pen_test_labels_normalized = set(normalize_label(l) for l in pen_test_labels)
    ...
    # Filter by pen test labels
    bugs_df = bugs_df[bugs_df['Labels'].notna()].copy()
    bugs_df['has_pen_test_label'] = bugs_df['Labels'].apply(
        lambda x: contains_pen_test_label(x, pen_test_labels_normalized)
    )
    bugs_df = bugs_df[bugs_df['has_pen_test_label']].copy()
    
    # If no pen test bugs found for this team, return zeros
    if len(bugs_df) == 0:
        return {
            'Weekly': 0,
            'Monthly': 0,
...
def k77(resources_file, jira_issues_file, output_dir, fiscal_start_month=4, current_date=None):
    """
    K77: Count penetration testing bugs at team level and generate k77-data.csv output.
    ...
    """
    output_file = os.path.join(output_dir, 'k77-data.csv')
    ...
    try:
        ...
        team_counts = {}
        for team in teams:
            counts = count_pen_test_bugs_by_team(
        ...
        print(f"  ✓ {total_teams_with_bugs} teams with penetration testing bugs")
        
        return True
        
    except Exception as e:
        print(f"  ✗ Error in K77: {e}")
        import traceback
        traceback.print_exc()
        return False
